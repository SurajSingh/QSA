...
namespace BL.Timesheet
{
    public class blTimesheet : DAL.clsDAL
    {
        ...
        public DataTable GetNewGeneratedPKID(string tableName, string columnName)
        {
            dbcommand = db.GetStoredProcCommand("uspGetNewGeneratedID", tableName, columnName);
        ...
        //**************************************************** dhaval excel changes **********************************************************
        public DataSet ImportExcel(Int64 FKManagerID, string SubmitType, string ApproveAction, string ApproveRemark, string TaskStatus, Int64 FKAssignLogID, object dtItem, Int64 FKCompanyID)
        {
            dbcommand = db.GetStoredProcCommand("uspimportExcel", FKManagerID, SubmitType, ApproveAction, ApproveRemark, TaskStatus, FKAssignLogID, dtItem, FKCompanyID, FKUserID, FKPageID, IPAddress, false);
            return db.ExecuteDataSet(dbcommand);
        }

        //**************************************************** Nilesh(s) excel changes **********************************************************
        public DataSet GetTimeSheetByDates(string fromDate, string toDate)
        {
            dbcommand = db.GetStoredProcCommand("uspGetTimeSheetByDates", fromDate, toDate);
            return db.ExecuteDataSet(dbcommand);
        }

        public DataSet ImportExcel(string parameterName, DataTable ExcelData)
        {
            dbcommand = db.GetStoredProcCommand("uspInsertExcelData");
            db.AddParameter(dbcommand, parameterName, SqlDbType.Structured, ParameterDirection.Input, "", DataRowVersion.Default, ExcelData);
            return db.ExecuteDataSet(dbcommand);
        }
