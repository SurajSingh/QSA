        ...
        public static string SaveData(Int64 PKID, object FromDate, object ToDate, Int64 FKEmpID, Int64 FKLeaveID, decimal LeaveCount, string Remarks)
        {
            ...
            if (status == 1)
            {
                ...
                if (dsResult.Tables[0].Rows.Count > 0 && Convert.ToInt64(dsResult.Tables[0].Rows[0]["Result"]) == 1)
                {
                    ...
                    string mailResponse = ObjSend.SendLeaveEmail(Convert.ToInt64(dsResult.Tables[0].Rows[0]["PKID"]), Convert.ToInt64(HttpContext.Current.Session["OrgID"]), "issue", (Int64)ClsPages.SMSEmailLocation.LeaveRequest, Convert.ToInt64(HttpContext.Current.Session["UserID"]), (Int64)ClsPages.WebPages.Schedule);
                    if (mailResponse != "1")
