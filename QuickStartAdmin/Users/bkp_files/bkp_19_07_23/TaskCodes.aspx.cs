...
{
    public partial class TaskCodes : System.Web.UI.Page
    {
        public string PageVersion = "15092022";
        public string StrCurrency = "$";
        protected void Page_Load(object sender, EventArgs e)
        {
            if (ClsLogin.ValidateRedirect((Int32)ClsRoles.UserRoles.TaskCodes, ""))
            {
                ((HiddenField)this.Master.FindControl("HidPageRoleID")).Value = ((Int32)ClsRoles.UserRoles.TaskCodes).ToString();
                ((HiddenField)this.Master.FindControl("HidPageID")).Value = ((Int32)ClsPages.WebPages.TaskCodes).ToString();
            }
        ...
        {
            string result = "";
            if (!ClsLogin.ValidateLogin() || !ClsLogin.ValidateRole((Int32)ClsRoles.UserRoles.TaskCodes, ClsRoles.IsView))
            {
        ...
        public static string GetData(Int64 PageSize, Int64 OffSet, string SortBy, string SortDir, Int64 PKID, string ActiveStatus, string TaskCode, Int64 FKDeptID)
        {
            ...
            else
            {
                blMaster objda = new blMaster();
                ClsGeneral objgen = new ClsGeneral();             
                DataSet ds = objda.GetTaskMaster(PageSize, OffSet, SortBy, SortDir,PKID, Convert.ToInt64(HttpContext.Current.Session["OrgID"]),TaskCode,FKDeptID,"T",ActiveStatus);
        ...
        public static string SaveData(Int64 PKID, string TaskCode, string TaskName, string Description, bool IsBillable, string ActiveStatus, Int64 FKDeptID, decimal CostRate, decimal BillRate, string TEType, decimal Tax, decimal BHours, bool isReimb, decimal MuRate)
        {
            ...
            if (status == 1)
            {
                if (!ClsLogin.ValidateLogin() || !ClsLogin.ValidateRole((Int32)ClsRoles.UserRoles.TaskCodes, PKID == 0 ? ClsRoles.IsAdd : ClsRoles.IsEdit))
                {
                    status = 0;
            ...
            if (status == 1)
            {
                ...
                objda.FKPageID = (Int64)ClsPages.WebPages.TaskCodes;
