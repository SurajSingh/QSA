...
var json;
var pagename = "ExpenseCodes.aspx";
var PKID = 0;
var PKRoleGroupID = 0;
var IsValidLogin = 0;
var FKPageID = 0;
...
function FunFillData(NewPageSize, OffSet, SortBy, SortDir, ExportType) {
    ...
    var args = {
        ...
        PKID: 0, ActiveStatus: $('#dropActiveStatusSrch').val(), TaskCode: $('#txtTaskCodeSrch').val(), FKDeptID: 0
        ...
        success: function (data) {
            ...
            if (data.d != "failure" && data.d != "Invalid Request") {
                ...
                if (jsonarr.data.Table.length > 0) {
                    if (jsonarr.data.Table[0].Result == "1") {
                        FunBlank();
                        $('#popupTitle').find('span').html("Modify Expense Code");
                        ...
                        $("#chkisReimb").prop('checked', jsonarr.data.Table[0].isReimb);
                        $("#dropActiveStatus").val(jsonarr.data.Table[0].ActiveStatus);
                        $("#txtBillRate").val(jsonarr.data.Table[0].BillRate);
                        $("#txtTax").val(jsonarr.data.Table[0].Tax);
                        $("#txtMuRate").val(jsonarr.data.Table[0].MuRate);
                        opendiv('divAddNew');
                    }
                    else {
                        OpenAlert(jsonarr[0].Msg);
...
function FunBlank() {
    ...
    $('#popupTitle').find('span').html("New Expense Code");
...
function FunValidate() {
    ...
    if ($("#txtTaskCode").val() == "") {
        ...
        strError += "<li>Enter Expense Type</li>";
    }
    else {
    ...
    if ($("#txtTaskName").val() == "") {
        ...
        strError += "<li>Enter Expense Name</li>";
    }
    else {
...
function FunSave() {
    ...
    var args = {
        PKID: PKID, TaskCode: $("#txtTaskCode").val(), TaskName: $("#txtTaskName").val(), Description: $("#txtDescription").val(), IsBillable: $("#chkIsBillable").is(':checked'), ActiveStatus: $("#dropActiveStatus").val(), FKDeptID: 0, CostRate: $("#txtCostRate").ValZero(), BillRate: 0,
        TEType: 'E', Tax: $("#txtTax").ValZero(), BHours: 0, isReimb: $("#chkisReimb").is(':checked'), MuRate: $("#txtMuRate").ValZero()
    };
...
function PageLoadFun() {
    ...
    SetAmountBox("txtMuRate", 2, false, "");
    SetAmountBox("txtTax", 2, false, "");
