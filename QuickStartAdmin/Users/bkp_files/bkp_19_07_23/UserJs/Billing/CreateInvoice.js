...
function FunSave() {
    ...
    var args = {
        PKID: PKID, InvDate: $('#txtInvDate').val(), FKProjectID: $('#txtFKProjectID').GenexAutoCompleteGet('0'), SNo: $('#txtSNo').ValZero(),
...
function FunFillProjectDetail(FKProjectID) {
    var args = {
        PKID: PKID, FKProjectID: FKProjectID
    };
            ...
            if (data.d != "failure" && data.d != "Invalid Request" && data.d != "") {
                ...
                if (jsonarr.data.Table7.length > 0) {
                    ...
                    $.each(jsonarr.data.Table7, function () {
                        ...
                        str = str + '<td>' + this.LoginID + '</td>';
                        str = str + '<td>' + this.TaskDate + '</td>';
                        str = str + '<td>' + this.TaskName + '</td>';
                        str = str + '<td style="text-align:right;">' + this.Hrs.toFixed(2) + '</td>';
                        str = str + '<td>' + this.Description + '</td>';
                        str = str + '<td style="text-align:right;">' + this.TBillRate.toFixed(2) + '</td>';
                        var amt = this.TBillRate * this.Hrs;
                        str = str + '<td style="text-align:right;">' + amt.toFixed(2) + '</td>';
                        str += '</tr>';

                        $('#tblTimeSheet tbody').append(str);

                        if (this.IsBilled == true) {
                ...
                if (jsonarr.data.Table8.length > 0) {
                    ...
                    $.each(jsonarr.data.Table8, function () {
                        ...
                        str = str + '<td>' + this.TaskDate + '</td>';
                        str = str + '<td>' + this.TaskName + '</td>';
                        str = str + '<td style="text-align:right;">' + this.Unit + '</td>';
                        str = str + '<td>' + this.Description + '</td>';
                        str = str + '<td style="text-align:right;">' + this.TCostRate.toFixed(2) + '</td>';
                        //var amt = this.TBillRate * this.Hrs;
                        str = str + '<td style="text-align:right;">' + this.Amount.toFixed(2) + '</td>';
                        str += '</tr>';
...
function InitilizeEvents() {
    ...
    $("#chkAllExpense").click(function () {
        var IsChecked = $('#chkAllExpense').is(':checked');
        $('#tblExpenseLog tbody tr').each(function (row, tr) {
            var trID = $(tr).attr("id");
            trID = trID.replace('trExpenseLog', '');
            $("#chkSelectExpense" + trID).prop("checked", IsChecked);
        });
    });
