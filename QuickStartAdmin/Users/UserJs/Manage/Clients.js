...
function FunFillData(NewPageSize, OffSet, SortBy, SortDir, ExportType) {
    ...
    var args = {
        ...
        PKID: 0, ActiveStatus: $('#dropActiveStatusSrch').val(), ClientName: $('#txtClientNameSrch').val()
...
function FunFillDetail() {
    ...
    var args = {
        ...
        PKID: PKID, ActiveStatus: '', ClientName:''
        ...
        success: function (data) {
            ...
            if (data.d != "failure" && data.d != "Invalid Request") {
                ...
                if (jsonarr.data.Table.length > 0) {
                    if (jsonarr.data.Table[0].Result == "1") {
                        FunBlank();
                        $('#popupTitle').find('span').html("Modify Client");
...
function FunBlank() {
    ...
    $('#popupTitle').find('span').html("New Client");
...
function FunValidate() {
    ...
    if ($("#txtCode").val() == "") {
        ...
        strError += "<li>Enter Client ID</li>";
    }
    else {
    ...
    if ($("#txtCompany").val() == "") {
        ...
        strError += "<li>Enter Client Name</li>";
    }
    else {
...
function FunSave() {
    ...
    var args = {
        PKID: PKID, FKManagerID: $("#txtFKManagerID").GenexAutoCompleteGet('0'), Code: $("#txtCode").val(), Company: $("#txtCompany").val(),
        CPerson: $("#txtCPerson").val(), CPersonTitle: $("#txtCPersonTitle").val(), ActiveStatus: $("#dropActiveStatus").val(),
