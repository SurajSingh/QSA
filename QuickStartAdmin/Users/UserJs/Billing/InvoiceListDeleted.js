...
var pagename = "InvoiceListDeleted.aspx";
var PKID = 0;
var FKPageID = 0;
var RCount = 0;
var JsonExport = null;
                    ...
                    if (jsonarr.data.Table[0].Result == "1") {
                        if (ExportType == null || ExportType == "") {
                            ...
                            $.each(jsonarr.data.Table, function () {
                                ...
                                str = str + '<td style="text-align:center">  <a class="linkRevokeRec text-danger" id="linkRevokeRec' + this.PKID + '"  title="Revoke Record"><i class="uil uil-file font-size-18"></i></a></td>';;
                            ...
                            $('#tbldata').on('click', '.linkRevokeRec', function (event) {
                                event.stopImmediatePropagation();
                                var newid = $(this).attr("id");
                                newid = newid.replace("linkRevokeRec", "");
                                FunRevoke(newid);
                            });
...
function FunDelete(RecID) {
    if (confirm("Do you want to delete this record Permenently?")) {
        var args = { PKID: RecID };
        ShowLoader();
        $.ajax({
                ...
                if (data.d != "failure") {
                    ...
                    if (jsonarr.length > 0) {
                        if (jsonarr[0].Result == "1") {
                            ...
                            OpenAlert("Record deleted Permanentely!");
                ...
                if (data.d != "failure") {
                    ...
                    if (jsonarr.length > 0) {
                        ...
                        if (jsonarr[0].Msg == "0") {
                            ...
                            //closediv();
                            OpenAlert('Invoice cannot Revoke, as the Task Entries are already billed with other invoiceId !' );
