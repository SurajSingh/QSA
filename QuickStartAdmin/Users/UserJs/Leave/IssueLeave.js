...
function FunFillData(NewPageSize, OffSet, SortBy, SortDir, ExportType) {
    ...
    var args = {
        ...
        LeaveType: $("#dropFKLeaveTypeIDSrch").val(), ApproveStatus:'Approved'
...
function FunSave() {
    ...
    var args = {
        PKID: PKID, FromDate: $("#txtFromDate").val(), ToDate: $("#txtToDate").val(), FKEmpID: $('#dropFKEmpID').ValZero(), FKLeaveID: $("#dropFKLeaveID").val(),
            ...
            if (data.d != "failure") {
                ...
                if (jsonarr.length > 0) {
                    ...
                    if (jsonarr[0].Result == "1") {
                        ...
                        HideLoader();
                        FunBlank();
                        OpenAlert('Leave Issued Successfully!');
...
function InitilizeEvents() {
    ...
    $("#btnAddNew").click(function () {
        if (IsAdd == 1) {
            FunBlank();
            opendiv("divSelectEmp");
        }
    });
    $("#btnNext").click(function () {
        if ($("#dropFKEmpID").ValZero() != '0') {
            FunFillEmpLeaveReport(false);
            $('#popupTitle').find('span').html("Issue Leave to " + $("#dropFKEmpID option:selected").text());
            closediv();
            opendiv("divAddNew");
        }
    });
