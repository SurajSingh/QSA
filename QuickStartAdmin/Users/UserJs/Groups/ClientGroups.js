...
var json;
var pagename = "/Users/ClientGroups.aspx";
var PKID = 0;
var jsonList = null;
...
function FunFillDetail(jsonarr) {
    FunBlank();
    $('#popupTitle').find('span').html("Modify Client Group");
...
function FunFillGroupDetail(jsonarr) {
    ...
    $.each(jsonarr.data.Table1, function (i, item) {
        str += '<tr><td><i class="fa fa-angle-right"></i>&nbsp;' + item.Company + '</td></tr>';
...
function FunBlank() {
    ...
    $('#popupTitle').find('span').html("New Client Group");
...
function PageLoadFun() {
    ...
    FunGetClientForAutoComplete(FunListCallback, 0, 'Active');
    InitilizeEvents();
    FunSetTabKey();
    /*FunFillData($("#tbldata").GenexTableGetPageSize(), 0, "", "D", "", "");*/
