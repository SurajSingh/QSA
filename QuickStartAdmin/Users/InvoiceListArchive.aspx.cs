...
{
    public partial class InvoiceListArchive : System.Web.UI.Page
    {
        ...
        public static string GetData(Int64 PageSize, Int64 OffSet, string SortBy, string SortDir, string daterange, string FromDate, string ToDate, Int64 PKID, string FKProjectID, string FKClientID, string InvoiceID)
        {
            ...
            else
            {
                ...
                DataSet ds = objda.GetInvoiceArchived(PageSize, OffSet, SortBy, SortDir, dresult.DateWise, dresult.fromdate, dresult.todate, PKID, Convert.ToInt64(HttpContext.Current.Session["OrgID"]), FKClientID, FKProjectID, InvoiceID,0);
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string RevokeArchivedData(Int64 PKID)
        {
            string result = "";
            if (!ClsLogin.ValidateLogin() || !ClsLogin.ValidateRole((Int32)ClsRoles.UserRoles.InvoiceList, PKID == 0 ? ClsRoles.IsAdd : ClsRoles.IsEdit))
            {
            ...
            else
            {
                ...
                objda.IPAddress = Classes.ClsLogin.GetRequestIP();
                objda.FKPageID = (Int64)ClsPages.WebPages.InvoiceList;
                DataSet dsResult = objda.RevokeArchiveInvoice(PKID, Convert.ToInt64(HttpContext.Current.Session["OrgID"]));
                result = objgen.SerializeToJSON(dsResult.Tables[0]);
