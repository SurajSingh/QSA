        ...
        public static string ApproveEmpLeaveRequest(Int64 PKID, string ApproveStatus, string Remarks)
        {
            ...
            else
            {
                ...
                if (dsResult.Tables[0].Rows.Count > 0 && Convert.ToInt64(dsResult.Tables[0].Rows[0]["Result"]) == 1)
                {
                    ...
                    string mailResponse = ObjSend.SendLeaveEmail(PKID, Convert.ToInt64(HttpContext.Current.Session["OrgID"]), "approve", (Int64)ClsPages.SMSEmailLocation.LeaveRequest, Convert.ToInt64(HttpContext.Current.Session["UserID"]), (Int64)ClsPages.WebPages.Schedule);
                    if (mailResponse != "1")
