CREATE PROCEDURE [dbo].[uspGetExpensesLog]
	@PageSize			bigint=0,
	@PageNo				Bigint=0,
	@SortBy				Varchar(200),
	@SortDir			Varchar(50),
...
Begin
	...
	Set @StrQry=N'	Select RCount = COUNT(*) OVER(),1 as Result,A.PKID, dbo.fnGetDateFormat(A.TaskDate,@DateForStr,''D'') as TaskDate, A.FKTaskID, A.FKEmpID, A.FKProjectID, A.Unit, A.Description, A.IsBillable, 
	A.Memo, A.MU, A.TCostRate,A.Amount,A.IsReimb,A.SavedFileName,A.OriginalFileName, A.ApproveStatus, 
	...
	B.LoginID,B.FName,B.LName,C.ProjectCode,C.ProjectName,Isnull(C2.EMemoRequired,0) as EMemoRequired,Isnull(C2.EDesReadonly,0) as EDesReadonly,
	...
	Begin
		If(@Remb='1')
		Begin	
			Set @StrQry=@StrQry+N' And A.IsReimb=1'
		End
		Else
		Begin	
			Set @StrQry=@StrQry+N' And A.IsReimb=0'
		End
	...
	SET @ParmDefinition = N'@PageSize bigint,@PageNo Bigint,@SortBy	Varchar(50),@SortDir Varchar(50),@DateWise	bit,@FromDate	Date,@ToDate Date,@PKID	Bigint,@FKCompanyID Bigint,	@FKEmpID Varchar(max),@FKClientID Varchar(max),	@FKProjectID Varchar(max),@FKTaskID	Varchar(max),@ApproveStatus Varchar(max),@Billed Varchar(50),@FKInvoiceID Bigint,@Remb varchar(50),@RecType	Varchar(50),@DateForStr Varchar(50),@FKTimezoneID Bigint'
	EXEC sp_executesql @StrQry, @ParmDefinition,@PageSize ,@PageNo ,@SortBy,@SortDir,@DateWise,@FromDate,@ToDate,@PKID,@FKCompanyID,	@FKEmpID,@FKClientID,	@FKProjectID,@FKTaskID,@ApproveStatus,@Billed,@FKInvoiceID,@Remb,@RecType,@DateForStr,@FKTimezoneID
