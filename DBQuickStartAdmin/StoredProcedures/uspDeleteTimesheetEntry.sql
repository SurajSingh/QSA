CREATE PROCEDURE [dbo].[uspDeleteTimesheetEntry]
	@PKID	Bigint,	
	@FKCompanyID	Bigint,	
	@FKUserID	Bigint,
	@FKPageID	Bigint=0,
...
Begin
	...
	Select @count=count(*) From tblTimeSheet Where PKID=@PKID and FKCompanyID=@FKCompanyID
	if(@count=0)
	Begin
		Set @Result=0
	...
	if(@Result=1)
	Begin
		SElect @Operation=Description From tblTimeSheet Where PKID=@PKID		
		
		Delete From tblTimeSheet Where  PKID=@PKID --Commented by Nilesh
		--update tblInvoice Set IsDeleted = 0 Where PKID=@PKID
		Set @Operation='Timesheet entry for '+@Operation+'  '
